...
blocks_result = db_conn.block_details.find({"height":{"$ne":0}}, {'tx': 1,'height':1},no_cursor_timeout=True)
for block in blocks_result:
    print '--------------------------- block : %s -------------------------' %block['height']
    ...
    try:
        for tx in transaction_list:
            ...
            db_conn.transaction_details.insert(insert_dict)
    except Exception as e:
        print 'Exception in inserting transaction:::: %s' %e.message
        pass
