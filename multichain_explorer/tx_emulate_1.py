...
def issue_assets():
    init_issue = custom_rpc('issue',[conf['root'],{"name":"aux_coin_v3","open":True},10000,0.01])
    txid_list = []
    priv_key_list = []
    address_list = []
    for i in range(0,10):
        keypair = custom_rpc('createkeypairs')
        issue_txid = custom_rpc('sendtoaddress',[keypair[0]['address'],{"aux_coin_v3":10}])
...
def create_transactions():
    while True:
        ...
        for i in range(0,len(txid_list)):
            ...
            hex_blob = custom_rpc('createrawtransaction',[inputs_list,{to_address:{"aux_coin_v3":5}}])
            ...
            end_time = (time.time())
            print 'txid is::::: %s for thread:: %s' %(txid,i)


            burn_coins(i,txid,to_address,new_keypair[0]['privkey'],txid_list[i],address_list[i],priv_key_list[i])
            # print 'txid for ::: %s is:: %s' %(counter,txid)
            _logging.debug('time taken for::: %s transaction with txid:: %s is::::: %s'  %(i,txid,end_time-start_time))
...
def burn_coins(counter,txid_1,address_1,priv_key_1,txid_2,address_2,priv_key_2):
    ...
    hex_blob = custom_rpc('createrawtransaction',[inputs_list,{conf['burn_address']:{"aux_coin_v3":5}}])
